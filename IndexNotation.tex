...
\definecolor{mycolorSubtle}{RGB}{230,240,250}
...
For complex Lorentz tensors, \lstinline|complexLorentzTensor.C| is equal to the type
\begin{code}
inductive Color
  | upL : Color
  | downL : Color
...
\lstinline|FD| from the discrete category formed by \lstinline|C| to 
the category of representations of \lstinline|G| over \lstinline|k|. This functor assigns to each color a representation of \lstinline|G| over \lstinline|k|.
...
the reader an idea of what each part does. Note that the \lstinline|Fermion.| part of the name 
of \lstinline|Fermion.leftHanded| is dropped in this definition, this is because it is inherented from 
...
The \lstinline|basis| part of the definition of a tensor species gives a basis indexed by \lstinline|Fin (repDim c)| 
(numbers from \lstinline|0| to \lstinline|repDim c - 1|)  of each representation for each \lstinline|c : C|.  
...
This natural transformation is simply the assignment to each color \lstinline|c| a linear-map from 
...
\begin{equation}
  ...
  \end{tikzpicture}
\end{equation}
where \lstinline|β_| is the braiding of the symmetric monoidal category, and \lstinline|e c| is shorthand for the isomorphism
in \lstinline|Discrete S.C| between \lstinline|c| and \lstinline|τ (τ c)|, which exists since \lstinline|τ| is an involution.

\sloppy Along with the contraction the definition of a tensor species includes the \lstinline|unit|, along with its own symmetry condition 
...
and it's interaction, \lstinline|contr_metric|, with the contraction and unit. 
...
For a given tensor species \lstinline|S|, the functor \lstinline|S.FD| can be lifted to a functor
to a symmetric monoidal functor from \lstinline|OverColor S.C| to \lstinline|Rep k G|.
Here the monoidal structure on \lstinline|Rep k G| is the tensor product over \lstinline|k|.
This functor takes \lstinline|f : X → S.C| to the tensor product over \lstinline|k| of all \lstinline|FD (f x)|,
\lstinline|⨂[k] x, S.FD (f x)|. 
...
\end{code}
from functors from \lstinline|Discrete S.C| to \lstinline|Rep S.k S.G|
to symmetric monoidal functors (or braided functors)  from \lstinline|OverColor S.C| to \lstinline|Rep S.k S.G|.

We denote the lift of \lstinline|S.FD| by \lstinline|S.F|, which is defined through
\begin{code}
...
For example for the tensor $\phi^{\mu}_{\phantom{\mu}\nu}$ would have \lstinline|X| as the type of indices which, since there are two of them, 
is \lstinline|Fin 2|, and \lstinline|f| as the function which assigns to each index the color of the index, 
so \lstinline|f 0| would be \lstinline|Color.up|, and \lstinline|f 1| would be \lstinline|Color.down|. 
We can apply the functor \lstinline|S.F| to \lstinline|f : X → S.C| in Lean as follows 
\lstinline|S.F.obj (OverColor.mk f)|, which we will abbreviate to \lstinline|S.F f|.
This representation, \lstinline|S.F f|, is the tensor product of each of representations \lstinline|S.FD (f x)| for \lstinline|x : X|. 
In our example this is equivalent to \lstinline|Lorentz.complexContr ⊕ Lorentz.complexCo|. Vectors of the form
...
\lstinline|S.F f| for some \lstinline|f : OverColor S.C|. 

In physics, we typically focus on objects \lstinline|f : X → S.C| of \lstinline|OverColor S.C|  where \lstinline|X| is a 
finite type of the form \lstinline|Fin n| for some \lstinline|n : ℕ|. 
...
\lstinline|TensorTree S c  → S.F c|, which is defined recursively as follows: 
\begin{code}
...
\paragraph{tensorNode:} The constructor \lstinline|tensorNode|
creates a tensor tree based on \lstinline|c| from a tensor \lstinline|t| in \lstinline|S.F c|. 
...
as can be seen in \lstinline|TensorTree.tensor|.
...
acting via \lstinline|c i| on \lstinline|0 ≤ i ≤ n -1|  and via \lstinline|c1 (i - n)| on \lstinline|n ≤ i ≤ (n + n1) - 1|.
...
giving a vector in \lstinline|S.F c ⊗ S.F c1|, using the tensorate of \lstinline|S.F| to map this vector into 
...
\lstinline|OverColor.mk (Sum.elim c c1 ∘ finSumFinEquiv.symm)| to map this vector into
 \lstinline|S.F (Sum.elim c c1 ∘ finSumFinEquiv.symm)|.
...
We start with a tensor in \lstinline|S.F c|.
This is mapped into a vector in \lstinline|(S.FD (c i) ⊗ S.FD (S.τ (c i))) ⊗ S.F (c ∘ Fin.succAbove i ∘ Fin.succAbove j)| via an equivalence from 
\lstinline|S.F c|. The equivalence is constructed using an equivalence 
...
\lstinline|𝟙 ⊗ S.F (c ∘ Fin.succAbove i ∘ Fin.succAbove j)| which is then 
mapped to a tensor in \lstinline|S.F (c ∘ Fin.succAbove i ∘ Fin.succAbove j)|
...
The tensor associated with the new tensor tree is constructed as follows.
We start with a tensor in \lstinline|S.F c|.
This is mapped into a vector in \lstinline|S.FD (c i) ⊗ S.F (c ∘ Fin.succAbove i)| via an equivalence from
\lstinline|S.F c|. The equivalence is constructed using an equivalence
...
\lstinline|𝟙 ⊗ S.F (c ∘ Fin.succAbove i)|.
...
We then finally map into \lstinline|S.F (c ∘ Fin.succAbove i)| 
...
If \lstinline|t| is a tensor tree and \lstinline|s| a subtree of \lstinline|t|, we can replace  \lstinline|s| in \lstinline|t| 
with another tensor tree \lstinline|s'|
to get a new overall tensor-tree \lstinline|t'|. If \lstinline|s| and \lstinline|s'| have the same underlying tensor, 
then \lstinline|t| and \lstinline|t'| will. 
...
to get the underlying tensor. We get this notation for free from the way \lstinline|TensorTree.tensor|
is defined with the prefix \lstinline|TensorTree.|.
...
under $SL(2, \mathbb{C})$ defined above as \lstinline|Lorentz.complexContr|.
...
defined above as \lstinline|Lorentz.complexCo|.
