...
giving a vector in \myinline|S.F c ⊗ S.F c1|, using the tensorate of \myinline|S.F| to map this vector into 
...
On the RHS here the first argument (\myinline!1!) of \myinline!contr! is the index of the first \myinline|ν| on the LHS, 
...
\myinline!{T | μ ν + T2 | ν μ}ᵀ!, provided of course that the indices are of the correct color (which Lean will check). 
...
where here the \myinline!_! is a placeholder for the permutation. For this case will the permutation will be the identity, but for 
...
Note the use of the \myinline!.tensor! to extract the tensor from the tensor tree, it does not really mean much 
