...
Let \lstinline|S| be a tensor species, and \lstinline|T : S.F (OverColor.mk ![c1, c2])| be a tensor with two indices 
of color \lstinline|c1| and \lstinline|c2|. The \lstinline|OverColor.mk| can be ignored here for the purpose 
of this conversation.

The syntax allows us to write the following 
\begin{code}
  {T | μ ν}ᵀ is elaborated as tensorNode T
\end{code}
...
\begin{center}
  ...
  $
  \begin{tikzpicture}
    \node[draw=black] (A) at (0,0) {S};
  \end{tikzpicture} = \begin{tikzpicture}
    \node[draw=black] (D1) at (-1,-3) {perm};
    \node[draw=black] (E1) at (-1,-4) {S};
    \path [->] (D1) edge (E1);
  \end{tikzpicture} 
$
  }
...
\begin{tabular}{|p{3in}| p{3in}|}
\hline
{\lstinline!rw [contr_tensor_eq <| contr_tensor_eq <| prod_tensor_eq_fst <| hA]!
}\newline 
\hrule~\newline
Description: Replaces the subtree {\lstinline|A|} with that specified by the RHS of {\lstinline|hA|}.
 & ~\newline
\makebox[3in]{%
\begin{tikzpicture}
  \node[draw=black] (A) at (0,0) {contr 0 0};
...
\begin{itemize}
\item Definition of Pauli matrices. 
\item Normally would explicity calculate this relation.
\item Way we do it is write the pauli matrices in a basis. 
\item One can define tensor tree relations involving basis. 
\item For example .... 
\item One can then use the sorts of thensor manipulations in Section ... 
\item To prove the relation.
\end{itemize}
\subsection{Example 3: Definitions of bispinors}
\begin{itemize}
\item We have defined bispinors using this formalism. 
\item And can use them to prove results. 
\end{itemize}
